...
class TestCollectionEmphasis(unittest.TestCase):
    def test_collection_from_crop_path(self):
        self.assertEqual(
            tid.collection_from_crop_path(
                "auchenorrhyncha/britishbugs/Acericerus_ribauti/x.jpg"
            ),
            "britishbugs",
        )
    ...
    def test_weights(self):
        self.assertAlmostEqual(
            tid.collection_weight("p/inaturalist/Taxon/a.jpg"), 0.8
        )
        self.assertAlmostEqual(
            tid.collection_weight("p/britishbugs/Taxon/a.jpg"), 1.2
        )
        self.assertAlmostEqual(tid.collection_weight("p/manual/Taxon/a.jpg"), 1.0)
        self.assertAlmostEqual(
            tid.collection_weight("p/inaturalist-manual/Taxon/a.jpg"), 1.0
        )

    def test_filter_excluded_labeled(self):
        labeled = [
            ("p/inaturalist/T/a.jpg", "Alpha"),
            ("p/truehopperswp/T/b.jpg", "Alpha"),
            ("p/britishbugs/T/c.jpg", "Beta"),
        ]
        out = tid.filter_excluded_collections(labeled)
        self.assertEqual(
            out,
            [
                ("p/inaturalist/T/a.jpg", "Alpha"),
    ...
    def test_filter_excluded_splits(self):
        splits = {
            "train": [
                {"crop_path": "p/truehopperswp/T/a.jpg", "genus": "Alpha"},
                {"crop_path": "p/britishbugs/T/b.jpg", "genus": "Alpha"},
            ],
            "val": [{"crop_path": "p/truehopperswp/T/c.jpg", "genus": "Alpha"}],
            "test": [{"crop_path": "p/manual/T/d.jpg", "genus": "Beta"}],
        }
        out = tid.filter_splits_excluded_collections(splits)
        self.assertEqual(
            out["train"],
            [{"crop_path": "p/britishbugs/T/b.jpg", "genus": "Alpha"}],
        )
        ...
        self.assertEqual(
            out["test"],
            [{"crop_path": "p/manual/T/d.jpg", "genus": "Beta"}],
        )

    def test_train_sample_weights_multiply_class_and_collection(self):
        records = [
            {"crop_path": "p/inaturalist/T/a.jpg", "genus": "A"},
            {"crop_path": "p/britishbugs/T/b.jpg", "genus": "A"},
        ]
        weights = tid.train_sample_weights(records)
        self.assertAlmostEqual(weights[1] / weights[0], 1.2 / 0.8)


class TestWeightedClassificationMetrics(unittest.TestCase):
    def test_top1_weights_britishbugs_more_than_inaturalist(self):
        y_true = [0, 0]
        y_pred = [0, 1]
        unweighted = tid.classification_metrics(y_true, y_pred, num_classes=1)
        weighted = tid.classification_metrics(
            y_true,
            y_pred,
            num_classes=1,
            sample_weights=[0.8, 1.2],
        )
        self.assertAlmostEqual(unweighted["top1"], 0.5)
        self.assertAlmostEqual(weighted["top1"], 0.8 / 2.0)
